	...
	<div class="container-fluid">
		<div class="row flex-xl-nowrap">
			<div class="col-md-3 col-lg-2 bg-light" id="left-nav-bar" style="padding-left:0; padding-right:0; background: #7107b8; height: 100%;">
				<div class="card">
					<div class="card" style="padding: 1rem;background-color:#b866ff; color: white;">
						...
						echo ('</p>');
						echo ('</p>');
						$query11 = "select t1.jid,jr.jobname,jr.salary FROM  STUD_GETS t1,job jr WHERE t1.sid={$_SESSION['uid']} and jr.jid=t1.jid ";
			...
			<main role="main" class="col-12 col-md-9 col-xl-8 py-md-3 pl-md-5 content">
				...
				#$view2="create or replace view stud_solve_req as select t1.jid, count(*) as courses_sat from job_req t1 join stud_course jr where jr.sid={$_SESSION['uid']} and jr.courseid=t1.courseid and jr.grade>t1.min_grade group by t1.jid "
				$sid = $_SESSION['uid'];
				// $query="select t2.jid as j, t3.jobname, t3.role, t3.salary from stud_course t1, job_req t2, job t3 where t1.sid={$sid} and t2.courseid=t1.courseid and t2.min_grade<=t1.grade and t2.jid=t3.jid group by t2.jid HAVING
				// count(*) in(select count(*) as cc from job_req group by jid having jid=j) order by t3.salary desc";

				$query = "SELECT jb.jid, jb.jobname, jb.role, jb.salary FROM job jb, stud_course sc, job_req jr WHERE sc.SID={$sid} AND jr.JID=jb.JID AND jr.COURSEID=sc.COURSEID AND sc.GRADE>=jr.MIN_GRADE ORDER BY salary desc";
